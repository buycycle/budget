...
setup_r: setup_env
	...
	@conda run -n $(ENV_NAME) Rscript -e "if (!requireNamespace('remotes', quietly = TRUE)) install.packages('remotes')"
	@conda run -n $(ENV_NAME) Rscript -e "packages <- readLines('requirements_R.txt'); \
	             for (pkg in packages) { \
	                 if (!requireNamespace(pkg, quietly = TRUE)) install.packages(pkg) \
	             }"
# Set up Python environment
setup_python: setup_env
